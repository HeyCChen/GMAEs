...
from mae.utils import create_activation
...
class GATConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        if self.res_fc is not None:
            ...
            # print(x_dst.shape) # 维度匹配问题
            # x_dst = x_dst.view(-1, self.heads * self.out_channels)
            # x_dst = x_dst.transpose(0, 1)
            # print(x_dst.shape)  # 维度匹配问题
            # print(x_ori.shape)
            # print(self.in_channels)
            # print(self.heads * self.out_channels)
            # resval = self.res_fc(x_dst).view(x_dst.shape[:-1], -1, self.out_channels)
            tmp = self.res_fc(x_ori)
            # print(tmp.shape)
            # print(x_ori.shape[:-1])
            # print(self.out_channels)
            # resval = self.res_fc(x_ori).view(x_ori.shape[:-1], -1, self.out_channels)
            resval = self.res_fc(x_ori).view(-1, H, C)
            # print(resval.shape)
            out = out + resval
        # print("OK!")
