...
class PreModel(nn.Module):
    ...
    def get_mask_node(self, x, mask_index, mask_rate=0.3):
        num_nodes = x.shape[0]

        perm = torch.randperm(num_nodes, device=x.device)
        keep_mask = torch.isin(perm, mask_index)
        edge_perm = perm[~keep_mask]

        edge_num_nodes = edge_perm.shape[0]
        num_mask_nodes = int(mask_rate * edge_num_nodes)
        mask_nodes = edge_perm[: num_mask_nodes]
        keep_edge = edge_perm[num_mask_nodes:]
        keep_nodes = torch.cat([mask_index, keep_edge])
    ...
    def forward(self, batch_g):
        ...
        loss = self.mask_attr_prediction(batch_g)
        loss_item = {"loss": loss.item()}
        return loss, loss_item

    def mask_attr_prediction(self, batch_g):
        x, edge_index = batch_g.x, batch_g.edge_index
        use_x, (mask_nodes, keep_nodes) = self.get_mask_node(
            x, batch_g.mask_index, self._mask_rate)
